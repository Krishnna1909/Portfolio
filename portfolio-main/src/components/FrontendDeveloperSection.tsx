...
export default function FrontendDeveloperSection() {
  ...
  useEffect(() => {
    ...
    if (!isDeletingText && typedText.length < currentPhrase.length) {
      timer = setTimeout(() => {
        setTypedText(currentPhrase.slice(0, typedText.length + 1));
      }, 100);
    } else if (!isDeletingText && typedText.length === currentPhrase.length) {
      timer = setTimeout(() => {
        setIsDeletingText(true);
      }, 2500);
    } else if (isDeletingText && typedText.length > 0) {
      timer = setTimeout(() => {
        setTypedText(currentPhrase.slice(0, typedText.length - 1));
      }, 50);
    } else if (isDeletingText && typedText.length === 0) {
  ...
  }, [typedText, isDeletingText, currentPhraseIndex]);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-2xl">
        ...
        <div className="min-h-[140px] sm:min-h-[190px] md:min-h-[220px] mb-6 flex items-center">
          <motion.h1
            initial={{ opacity: 0, scale: 0.85, y: 30 }}
            animate={inView ? { opacity: 1, scale: 1, y: 0 } : { opacity: 0, scale: 0.85, y: 30 }}
            transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}
            className="font-extrabold leading-[1.1] tracking-tight text-white text-[clamp(44px,7.5vw,96px)] select-text"
          >
            <span>
              {typedText}
              <span className="inline-block w-[4px] md:w-[8px] h-[0.8em] bg-white ml-2 align-middle animate-pulse" />
            </span>
          </motion.h1>
        </div>
    ...
    animate-[shine_4s_linear_infinite]"
        ...
        >
          {portfolioData.skills.hero.map((tech) => (
            <div
              key={tech}
              className="
      ...
      "
        ...
        <div className="mt-8 flex flex-col [@media(min-width:540px)]:flex-row items-start md:items-center gap-4">
          ...
          <motion.button
            ...
            className="inline-flex items-center gap-2 border border-accent text-accent px-6 py-3 text-xs tracking-[0.25em] uppercase font-semibold hover:bg-accent hover:text-black transition-all duration-200 rounded-full relative z-20 cursor-pointer"
          >
