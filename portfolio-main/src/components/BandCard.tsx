...
const PROFILE_PATH = "/assets/krishna-profile.png";
...
function Band({
  ...
  // Dynamic canvas textures matching 1024x1024 GLTF UV coordinates
  const { cardCanvasTexture, lanyardStrapTexture } = useMemo(() => {
    // 1. Card Texture (1024x1024) - Front face UV bounds (x:0..540, y:0..750)
    const cardCanvas = document.createElement("canvas");
    cardCanvas.width = 1024;
    cardCanvas.height = 1024;
    const ctx = cardCanvas.getContext("2d");

    if (ctx && profileTexture.image) {
      const img = profileTexture.image;
      const cardW = 540;
      const cardH = 750;

      // Fill entire card texture with dark studio background color
      ctx.fillStyle = "#18171a";
      ctx.fillRect(0, 0, 1024, 1024);

      // Fit photo perfectly so face and shoulders are centered without cropping
      ...
      if (imgAspect > cardAspect) {
        // Wider image: match height, center horizontally
        drawH = cardH;
        drawW = cardH * imgAspect;
        drawX = (cardW - drawW) / 2;
      } else {
        // Taller image: match width, center vertically
        drawW = cardW;
    ...
    // 2. Lanyard Strap Ribbon Texture (ONLY "Krishna")
    ...
    if (sCtx) {
      ...
      sCtx.fillRect(0, 0, 512, 6);
      sCtx.fillRect(0, 122, 512, 6);

      sCtx.fillStyle = "#ffffff";
      sCtx.font = "bold 44px 'Segoe UI', sans-serif";
      sCtx.textAlign = "center";
      sCtx.fillText("Krishna", 256, 75);
    }
    ...
    return { cardCanvasTexture: cardTex, lanyardStrapTexture: strapTex };
  }, [profileTexture]);
